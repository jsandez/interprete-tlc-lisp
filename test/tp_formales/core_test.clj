...
(deftest evaluar-de-test
  (testing "Prueba de la funcion: evaluar-de"
    ...
    (is (= (evaluar-de '(de f (x y) (+ x y)) '(x 1 y 1)) '(f (x 1 y 1 f (lambda (x y) (+ x y))))))
    (is (= (evaluar-de '(de f (X Y) (+ X Y)) '(X 1 Y 1)) '(f (x 1 y 1 f (lambda (x y) (+ x y))))))
...
(deftest evaluar-or-test
  (testing "Prueba de la funcion: evaluar-or"
    ...
    (is (= (evaluar-or '(or (gt 0 2) (setq b 8) nil) '(gt gt nil nil t t w 5 x 4) '(x 1 y nil z 3)) '(8 (gt gt nil nil t t w 5 x 4 b 8))))
